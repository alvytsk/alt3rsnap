...
>
> **Two cross-cutting adjustments apply to every Windows module in Phase 4+** (discovered during Task 21 implementation):
>
> 1. **`crate::engine::...` → `alt3rsnap::engine::...`** — Windows adapter modules (`win_api.rs`, `hook.rs`, `adapter.rs`, `tray.rs`, `fullscreen.rs`, `elevate.rs`, `autostart.rs`, etc.) live in the **binary** crate under `src/`, declared via `mod X;` in `src/main.rs`. `engine` lives in the **library** crate (declared in `src/lib.rs`). From the binary crate, you cannot reach library modules via `crate::engine::X` — use the external path `alt3rsnap::engine::X`.
> 2. **HWND pointer semantics in `windows = "0.58"`** — `HWND.0` is `*mut core::ffi::c_void`, not `isize`. Apply these patterns:
>    - Null check: `hwnd.0.is_null()` (not `hwnd.0 == 0`).
>    - HWND → u64: `hwnd.0 as usize as u64` (two-step cast; `as u64` from a pointer is disallowed).
>    - u64 → HWND: `HWND(id.0 as usize as *mut core::ffi::c_void)`.
>
> Subagent implementers: apply these patterns silently when the plan text shows the older form — they're scripted around Rev-1 plan text, not around code-review corrections.
